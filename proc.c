...
found:
  ...
  p->pid = nextpid++;
  p->priority = 10;
  p->tickets = random(p->priority*100);
  p->itr = 0;
...
acquire(&ptable.lock);
cprintf("name \t pid \t state \t priority \t tickets \t itr\n");
for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
  if(p->state == SLEEPING)
	  cprintf("%s \t \t %d \t SLEEPING \t \t %d \t %d \t %d \n ", p->name,p->pid,p->priority,p->tickets,p->itr);
